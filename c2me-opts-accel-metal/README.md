...
- startup validation that exercises DFC -> MSL -> Metal, 257-element batching, pipeline reuse, buffer reuse, the explicit F32 boundary, and a nested spline raw-bit differential probe.
...
`MetalF32SplinePlanner` now applies this policy to `SplineNormalNode`: every location function remains an F64 boundary producer, repeated producer objects share a boundary slot, and nested spline values are planned recursively only when their result is already F32. This is intentionally separate from the conservative direct-AST compatibility gate.

The startup probes verify 257 host-rounded values across the generic F64 -> F32 boundary and also build a nested spline containing two independent F64 boundary producers. The Java reference and Metal result must match at the raw binary32 bit level for every sample. This validates the implemented spline island contract without claiming that arbitrary F32 DensityFunction arithmetic is already portable to Metal.
...
This is necessary but not considered sufficient proof of world-generation determinism. The nested spline runtime probe is therefore a hard availability gate: if Metal changes the audited operation sequence in a way that changes the expected raw F32 bits, initialization fails and C2ME continues on its normal path.
...
2. The host supplies slot-major boundary values only after exact-path evaluation and an explicit `(float)` conversion.
3. `MetalF32SplineReference` evaluates the plan on Java using the same binary range search, outside-range sampling and interpolation expression ordering as the current OpenCL spline runtime/emitter.
4. `MetalF32SplineCompiler` emits the same planned tree as MSL, including nested spline values and raw-bit encoded locations/derivatives.
5. `MetalSplineRuntimeProbe` compares Java-reference and Metal results bit-for-bit across 257 samples and also verifies pipeline/buffer reuse.
...
The next integration problem is no longer how to represent spline math; it is how to evaluate and batch real F64 boundary producers at a profitable world-generation boundary without adding per-sample CPU/GPU synchronization. The first real dispatch must therefore be chunk/region-batched rather than a tiny AST-call offload.
...
1. Identify the real world-generation call site where F64 spline boundary producers can be evaluated in batches without changing their exact semantics.
2. Build the host adapter that evaluates those producers on the existing exact path, performs the explicit F64 -> F32 conversion, and fills slot-major Metal batch inputs.
